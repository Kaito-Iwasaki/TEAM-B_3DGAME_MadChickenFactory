//=====================================================================
//
// モーションスクリプト読み込み処理 [motion_loader.cpp]
...
//=====================================================================
// [PARTSET]読み込み処理
// pFileからパーツとして設定するモデルのパラメータを
// PART構造体のポインタ"pBuffer"に読み込みます。
//=====================================================================
void _Read_PARTSSET(FILE* pFile, PART* pBuffer)
{
	char aStrLine[MAX_READABLE_CHAR] = {};
	...
	while (true)
	{
		...
		{
			fscanf(pFile, " = %f %f %f", &pBuffer->posOffset.x, &pBuffer->posOffset.y, &pBuffer->posOffset.z);
		}
		if (strcmp(&aStrLine[0], "ROT") == 0)
		{
			fscanf(pFile, " = %f %f %f", &pBuffer->rotOffset.x, &pBuffer->rotOffset.y, &pBuffer->rotOffset.z);
		}
	}
}
...
// [MOTIONSET]読み込み処理
// pFileモーション情報を
// MOTION_INFO構造体のポインタ"pBuffer"に読み込みます。
//=====================================================================
void _Read_MOTIONSET(FILE* pFile, MOTION_INFO* pBuffer)
{
	char aStrLine[MAX_READABLE_CHAR] = {};
