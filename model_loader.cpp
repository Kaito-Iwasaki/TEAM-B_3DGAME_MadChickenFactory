...
void _Read_FIRESET(FILE* pFile, FIRESETDATA* pBuffer)
{
	...
	while (true)
	{
		...
		else if (strcmp(&aStrLine[0], "SWITCH") == 0)
		{
			fscanf(pFile, " = %d", &pBuffer->nCntSwitch);
		}
		else if (strcmp(&aStrLine[0], "LIFE") == 0)
		{
			fscanf(pFile, " = %d", &pBuffer->nLife);
		}
	}
}
...
void _Read_LIFTSET(FILE* pFile, LIFTSETDATA* pBuffer)
{
	...
	while (true)
	{
		...
		if (strcmp(&aStrLine[0], "END_LIFTSET") == 0)
		{
			break;
		}
		else if (strcmp(&aStrLine[0], "ID") == 0)
